...
export class ProfileComponent {
    ...
    if (service.name.trim() && service.price > 0) {
      if (service.id) {
        ...
        this.updateServiceInDatabase(service).then(success => {
          if (!success) {
            alert('No se pudo actualizar el servicio. Los cambios se guardarán cuando guarde todo el perfil.');
          }
        });
      } else {
  ...
  async save(){
    ...
    const { error } = await this.sb.supabase.from('profiles').update({
